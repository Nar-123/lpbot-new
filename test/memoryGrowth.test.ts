 ...
 *
 * Phase 4.8 added a fourth: chain/clients.ts's `accountCache` (F-10 from the
 * Phase 4.7 audit) — one permanent key per distinct wallet id ever
 * activated, found by that audit's accumulator inventory as the only
 * module-level cache in src/ left unbounded after the other three above
 * were fixed.
 *
...
import {
  __setAccountCacheEntryForTests,
  __accountCacheSizeForTests,
  __accountCacheHasForTests,
...
// ── chain/clients.ts accountCache (Phase 4.8 / F-10) ───────────────────────

test('accountCache: long-run simulation — 10,000 distinct wallet ids stay bounded at the configured max size', () => {
  __resetAccountCacheForTests();
  for (let i = 0; i < 10_000; i++) {
    __setAccountCacheEntryForTests(`wallet-${i}`, fakeAccount);
  }
  assert.equal(__accountCacheSizeForTests(), 500, 'bounded at MAX_ACCOUNT_CACHE_SIZE regardless of lifetime insert count');
});

test('accountCache: FIFO eviction drops the oldest wallet id first, never a recently-added one', () => {
  __resetAccountCacheForTests();
  for (let i = 0; i < 500; i++) {
    __setAccountCacheEntryForTests(`k${i}`, fakeAccount);
  }
  assert.ok(__accountCacheHasForTests('k0'));
  __setAccountCacheEntryForTests('k500', fakeAccount);
  assert.equal(__accountCacheSizeForTests(), 500);
  assert.ok(!__accountCacheHasForTests('k0'), 'oldest key evicted');
  assert.ok(__accountCacheHasForTests('k1'), 'next-oldest survives');
  assert.ok(__accountCacheHasForTests('k500'), 'newly-added key present');
});

test('accountCache: re-caching an already-cached wallet id does not consume a slot / trigger eviction', () => {
  __resetAccountCacheForTests();
  for (let i = 0; i < 500; i++) {
    __setAccountCacheEntryForTests(`k${i}`, fakeAccount);
  }
  __setAccountCacheEntryForTests('k0', fakeAccount);
  assert.equal(__accountCacheSizeForTests(), 500, 'updating an existing key must not evict anything');
  assert.ok(__accountCacheHasForTests('k0'));
  assert.ok(__accountCacheHasForTests('k1'));
});
