...
test('computeLift is positive when higher normalized values coincide with wins', () => {
  const records: PerformanceRecord[] = [
    rec({ signals: { volumeUsd: 100 }, realizedUsd: 50 }), // win, high
    rec({ signals: { volumeUsd: 90 }, realizedUsd: 40 }), // win, high
    rec({ signals: { volumeUsd: 10 }, realizedUsd: -20 }), // loss, low
    rec({ signals: { volumeUsd: 5 }, realizedUsd: -10 }), // loss, low
  ];
  const lift = computeLift(records, 'volumeUsd');
...
test('recalculateWeights leaves weights completely unchanged with too few samples', () => {
  const current: SignalWeights = { volumeUsd: 0.3, poolTvlUsd: 0.3 };
  const records: PerformanceRecord[] = [
    rec({ signals: { volumeUsd: 100 }, realizedUsd: 50 }),
    rec({ signals: { volumeUsd: 10 }, realizedUsd: -20 }),
...
test('recalculateWeights boosts the top-lift signal and decays the bottom-lift signal, leaving the middle untouched', () => {
  const current: SignalWeights = {
    volumeUsd: 0.3, // will have strong POSITIVE lift (high in wins)
  ...
  const result = recalculateWeights(current, records);
  assert.ok(result.volumeUsd! > current.volumeUsd!, 'positive-lift signal should be boosted');
...
test('recalculateWeights never pushes a weight below the configured floor', () => {
  const current: SignalWeights = { volumeUsd: 0.3, poolTvlUsd: 0.06, poolVolumeUsd: 0.2, ageHours: 0.2 };
  const records: PerformanceRecord[] = Array.from({ length: 12 }, (_, i) => {
    const isWin = i < 6;
    return rec({
      signals: {
...
test('recalculateWeights never pushes a weight above the configured ceiling', () => {
  const current: SignalWeights = { volumeUsd: 0.9, poolTvlUsd: 0.2, poolVolumeUsd: 0.2, ageHours: 0.2 };
  const records: PerformanceRecord[] = Array.from({ length: 12 }, (_, i) => {
    const isWin = i < 6;
    return rec({
      signals: {
        volumeUsd: isWin ? 100 + i : 10 + i, // strong positive lift — repeated boost would breach the ceiling
        poolTvlUsd: 50,
        poolVolumeUsd: 50,
        ageHours: 50,
      },
  ...
  assert.ok(weights.volumeUsd! <= 1.0 + 1e-9, `weight ${weights.volumeUsd} exceeded the ceiling`);
});

test('recalculateWeights never invents a weight for a signal absent from currentWeights', () => {
  const current: SignalWeights = { volumeUsd: 0.3, poolTvlUsd: 0.3, poolVolumeUsd: 0.2, ageHours: 0.2 };
  // poolFee is never in currentWeights — even with a full, strongly-differentiated dataset.
  const records: PerformanceRecord[] = Array.from({ length: 12 }, (_, i) => {
    const isWin = i < 6;
    return rec({
      signals: { volumeUsd: isWin ? 100 : 10, poolFee: isWin ? 3000 : 500 },
      realizedUsd: isWin ? 50 : -20,
    });
  });
  const result = recalculateWeights(current, records);
...
test('with fewer than 4 scoreable signals, weights are left untouched (quartile split is not meaningful)', () => {
  const current: SignalWeights = { volumeUsd: 0.5, poolTvlUsd: 0.5 };
  const records: PerformanceRecord[] = Array.from({ length: 12 }, (_, i) => {
    const isWin = i < 6;
    return rec({ signals: { volumeUsd: isWin ? 100 : 10 }, realizedUsd: isWin ? 50 : -20 });
  });
  const result = recalculateWeights(current, records);
  assert.deepEqual(result, current);
});
