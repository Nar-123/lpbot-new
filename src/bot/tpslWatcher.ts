...
async function executeClose(
  ...
  const key = posKey(p);
  const lockKey = closeLockKey(p.chainId, p.tokenId);
  if (!tryAcquireCloseLock(lockKey)) {
    console.warn(`[tpsl] close skipped ${key} — position sedang ditutup oleh proses lain (manual /close atau TP/SL lain)`);
    return;
  }
  try {
...
async function recheckAndMaybeClose(
  ...
  if (isCloseLocked(closeLockKey(p.chainId, p.tokenId))) {
    // Held by a concurrent close (e.g. a manual /close already in
    // progress on this exact position) — don't confirm, don't unenroll.
    // Leave the pending trigger in place, same as the 'unknown' PnL-read
    // case below: the next regular tick re-evaluates once the lock clears.
    console.warn(`[tpsl] recheck ${key} skipped — position sedang ditutup oleh proses lain`);
    return;
  }
...
export function stopTpslWatcher(): Promise<void> {
  ...
  shutdownPromise = (async () => {
    ...
    // Note: the close lock itself is NOT cleared here. It is shared with
    // bot.ts's manual /close path (Phase 4.7 finding), and each close
    // (including one still running past the deadline above) releases its
    // own lock key in its own `finally` when it actually finishes —
    // clearing it here could otherwise release a lock still legitimately
    // held by a close that outlived the shutdown deadline.
...
export function __isClosingForTests(chainId: number, tokenId: string): boolean {
  return isCloseLocked(closeLockKey(chainId, tokenId));
}
