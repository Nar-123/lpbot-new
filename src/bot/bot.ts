...
async function executeClosePosition(
  ...
  const lockKey = closeLockKey(chainId, tokenId.toString());
  if (!tryAcquireCloseLock(lockKey)) {
    await ctx.reply(
      `⏳ Posisi #${tokenId} sedang ditutup oleh TP/SL — tunggu sampai selesai lalu cek /list.`,
    );
    return;
  }
  try {
    await executeClosePositionLocked(ctx, chainId, protocol, tokenId, prefs, dex);
  } finally {
    releaseCloseLock(lockKey);
  }
}

async function executeClosePositionLocked(
  ctx: Context,
  chainId: SupportedChainId,
  protocol: 'v3' | 'v4',
  tokenId: bigint,
