...
/**
 * Phase 4.8 fix: this was the only module-level cache in src/ with no
 * bound/eviction (unlike metaCache/supplyCache in tokens.ts and priceCache
 * in dexscreener.ts, all bounded since Phase 4.6.8/4.6.9). A derived
 * `Account` never changes for a given wallet id once computed, so — exactly
 * like tokens.ts's metaCache — bounding by count rather than TTL is
 * correct: an evicted entry just costs one more (cheap, local, no RPC)
 * `privateKeyToAccount` call on next use, never a different or incorrect
 * result. FIFO via Map's insertion-order iteration, mirroring
 * setMetaCacheBounded exactly.
 */
const MAX_ACCOUNT_CACHE_SIZE = 500;

const RPC_TIMEOUT_MS = 12_000;

function setAccountCacheBounded(key: string, acc: Account): void {
  if (!accountCache.has(key) && accountCache.size >= MAX_ACCOUNT_CACHE_SIZE) {
...
function accountForWallet(w: StoredWallet): Account {
  ...
  if (!acc) {
    ...
    setAccountCacheBounded(w.id, acc);
  }
  return acc;
}

/** Test-only: exercise the bounded-cache logic directly, without a real wallet/key. */
export function __setAccountCacheEntryForTests(key: string, acc: Account): void {
  setAccountCacheBounded(key, acc);
}
export function __accountCacheSizeForTests(): number {
  return accountCache.size;
}
export function __accountCacheHasForTests(key: string): boolean {
  return accountCache.has(key);
}
