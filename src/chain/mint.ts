...
import { releaseMintLock, tryAcquireMintLock } from './mintLock.js';
...
export async function mintSingleSided(params: MintParamsWithProtocol): Promise<MintResult> {
  ...
  // Phase 4.8 fix: closes the TOCTOU between the balance read used for
  // sizing (below) and the actual broadcast — see mintLock.ts's doc
  // comment. A concurrent v3 mint (manual or MULTI) on the same chain is
  // rejected outright rather than silently sizing from the same stale
  // balance.
  const lockChainId = params.chainId;
  if (!tryAcquireMintLock(lockChainId)) {
    throw new Error(
      `A mint is already in progress on chain ${lockChainId} — wait for it to finish before starting another (prevents sizing from a stale wallet balance).`,
    );
  }
  try {
    return await mintSingleSidedV3Locked(params);
  } finally {
    releaseMintLock(lockChainId);
  }
}

async function mintSingleSidedV3Locked(params: MintParamsWithProtocol): Promise<MintResult> {
  const {
    chainId,
    poolAddress,
    balancePercent,
