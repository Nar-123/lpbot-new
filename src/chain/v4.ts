...
export async function mintV4SingleSided(params: V4MintParams): Promise<V4MintResult> {
  ...
  // comment. Shares the same per-chain lock as mint.ts's v3 path, so a
  // concurrent v3 and v4 mint on the same chain (same hot wallet) cannot
  ...
  try {
    return await mintV4SingleSidedLocked(params);
  } finally {
    releaseMintLock(lockChainId);
  }
}

async function mintV4SingleSidedLocked(params: V4MintParams): Promise<V4MintResult> {
  const {
    chainId,
    poolId,
    poolKey,
