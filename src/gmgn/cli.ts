...
/**
 * Validates and unwraps the `market trending` response shape (a bare array,
 * or `{ rank: [...] }`). Exported separately from `gmgnMarketTrending` so
 * this shape check is unit-testable without a real CLI invocation — mirrors
 * price/dexscreener.ts's parse* boundary validators.
 *
 * Phase 4.8 fix: a well-formed-but-wrong-shape response (a schema rename,
 * `{}` without `.rank`, `.rank` present but not an array, ...) must be
 * distinguishable from "the source responded and genuinely found nothing
 * today" — collapsing both into `[]` bypassed the sourceError mechanism
 * entirely (multiCandidates.ts's fetchAndFilterCandidates only classifies a
 * THROWN error as a source failure). Throwing here routes a schema change
 * through that same, already-alerted path instead of silently degrading to
 * "0 candidates today".
 */
export function parseTrendingResponse(data: unknown): GmgnTrendingToken[] {
  if (Array.isArray(data)) return data as GmgnTrendingToken[];
  if (data && typeof data === 'object' && Array.isArray((data as { rank?: unknown }).rank)) {
    return (data as { rank: GmgnTrendingToken[] }).rank;
  }
  throw new GmgnError(
    `gmgn-cli market trending returned an unrecognized shape (expected an array or {rank: [...]}): ${String(JSON.stringify(data)).slice(0, 300)}`,
    undefined,
    'GMGN_CLI_MALFORMED_OUTPUT',
  );
}
